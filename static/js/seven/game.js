...
(function () {
  ...
  window.SS.Lobby.init({
    youId,
    fields: [
      { key: "max_score", label: "Max score (out)", min: 20, max: 1000 },
      { key: "stop_penalty", label: "Stop penalty", min: 0, max: 200 },
      { key: "win_discount", label: "Win discount", min: 0, max: 50 },
      { key: "turn_timer", label: "Turn timer (s)", min: 15, max: 180 },
      { key: "timeout_limit", label: "Timeouts allowed", min: 1, max: 10 },
      { key: "num_decks", label: "Number of decks", min: 1, max: 10 },
    ],
  });
